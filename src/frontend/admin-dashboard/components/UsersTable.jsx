 ...
 */
export function UsersTable({ users, onEdit, onDelete, onUserClick }) {
    if (users.length === 0) {
        return (
            ...
            <table className="w-full min-w-[620px] border-collapse text-left text-sm">
                ...
                <tbody>
                    {users.map((user) => (
                        <tr
                            key={user.id}
                            className="border-b border-[var(--border-subtle)] transition last:border-0 hover:bg-[var(--bg-hover)] cursor-pointer"
